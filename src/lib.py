...
def create_response(response, username):
    ...
    expires = datetime.now() + timedelta(days=1)  # Set the expiration time to 1 days from now
...
def mfa_is_enabled(username):
    database = "db_users.sqlite"
    query = "SELECT * FROM users WHERE username = :username and mfa_enabled = 1"
    data = {
        "username": username
    }
...
def mfa_disable(username):
    database = "db_users.sqlite"
    query = "UPDATE users SET mfa_enabled = 0 WHERE username = :username"
    data = {
        "username": username
    }
...
def mfa_enable(username):
    database = "db_users.sqlite"
    query = "UPDATE users SET mfa_enabled = 1 WHERE username = :username"
    data = {
        "username": username
    }
...
def mfa_reset_secret(username):
    ...
    database = "db_users.sqlite"
    query = "UPDATE users SET mfa_secret = :secret WHERE username = :username"
    data = {
        "secret": secret,
        "username": username
    }
